...
class NavBar extends Component<IProps, PageState> {
  ...
  render() {
    ...
    if (title) {
      ...
      nav_bar__center = (
        <View
          className='qz-nav-bar-search'
          style={`height:${capsulePosition.height}px;`}
          onClick={this.handleSearchClick.bind(this)}
        >
          <View className='qz-nav-bar-search__icon'/>
          <View className='qz-nav-bar-search__input'>{searchText}</View>
        </View>
      );
    } else {
      /* eslint-disable */
    ...
    return (
      <View
        className={`qz-nav-bar ${ios ? 'ios' : 'android'} ${extClass}`}
        style={`background: ${backgroundColorTop ? backgroundColorTop : background};height:${navBarHeight +
        navBarExtendHeight}px;`}
      >
        <View
          className={`qz-nav-bar__placeholder ${ios ? 'ios' : 'android'}`}
          style={`padding-top: ${navBarHeight + navBarExtendHeight}px;`}
        />
        <View
          className={`qz-nav-bar__inner ${ios ? 'ios' : 'android'}`}
          style={`background:${background};${navigationbarinnerStyle};`}
        >
          <View className='qz-nav-bar__left' style={navBarLeft}>
            {back && !home && (
              <View
                onClick={this.handleBackClick.bind(this)}
                className={`qz-nav-bar__button qz-nav-bar__btn_goback ${iconTheme}`}
              />
            )}
            {!back && home && (
              <View
                onClick={this.handleGoHomeClick.bind(this)}
                className={`qz-nav-bar__button qz-nav-bar__btn_gohome ${iconTheme}`}
              />
            )}
            {back && home && (
              <View className={`qz-nav-bar__buttons ${ios ? 'ios' : 'android'}`}>
                <View
                  onClick={this.handleBackClick.bind(this)}
                  className={`qz-nav-bar__button qz-nav-bar__btn_goback ${iconTheme}`}
                />
          ...
          <View className='qz-nav-bar__center' style={`padding-left: ${rightDistance}px`}>
            {nav_bar__center}
          </View>
          <View className='qz-nav-bar__right' style={`margin-right: ${rightDistance}px`}>
            <AtButton
              className="qz-nav-bar__right-feeback vertical-middle"
              size="small"
              type="primary"
              full
              circle
